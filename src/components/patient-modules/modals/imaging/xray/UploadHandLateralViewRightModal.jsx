...
const UploadHandLateralViewRightModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		if (image) {
			formData.append("hand_lateral_view_right", dataURItoBlob(image));
		}
		// return "";
		Axios.post(
			`v1/imaging/hand-lateral-view-right-img-result/${showData?.id}`,
			formData,
			config
		)
			.then((res) => {
