...
const UploadThoracicBonyCageAPPortableFeeModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		if (image) {
			formData.append("thoracic_bony_cage_ap_pf", dataURItoBlob(image));
		}
		// return "";
		Axios.post(
			`v1/imaging/thoracic-bony-cage-ap-pf-img-result/${showData?.id}`,
			formData,
			config
		)
			.then((res) => {
