...
const UploadLumbosacralSpineAPLObliquePortableFeeModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		if (image) {
			formData.append("lumbosacral_spine_apl_oblique_pf", dataURItoBlob(image));
		}
		// return "";
		Axios.post(
			`v1/imaging/lumbosacral-spine-apl-oblique-pf-img-result/${showData?.id}`,
			formData,
			config
		)
			.then((res) => {
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									<div className="border-b">
										<span className="text-xl text-center font-bold  text-blue-500">
                                         LUMBOSACRAL SPINE APL + OBLIQUE + PORTABLE FEE
                                            </span>
									</div>
									<Controller
										name="image"
