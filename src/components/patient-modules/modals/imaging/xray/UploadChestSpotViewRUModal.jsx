...
const UploadChestSpotViewRUModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		if (image) {
			formData.append("chest_spot_view_right_ulf", dataURItoBlob(image));
		}
		// return "";
		Axios.post(
			`v1/imaging/chest-spot-view-right-ulf-img-result/${showData?.id}`,
			formData,
			config
		)
			.then((res) => {
	...
	const noHide = () => {};
	const filteredPending = pending?.data.filter(order => order.type?.name === "Ultrasound");  return (
