...
const UploadCervicalAplObliquesModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		if (image) {
			formData.append("cervical_apl_obliques", dataURItoBlob(image));
		}
		// return "";
		Axios.post(
			`v1/imaging/cervical-apl-obliques-img-result/${showData?.id}`,
			formData,
			config
		)
			.then((res) => {
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									...
									<Controller
										...
										// errors={errors?.rhu_xray_result?.message}
												...
												{hasError("cervical_apl_obliques") ? (
