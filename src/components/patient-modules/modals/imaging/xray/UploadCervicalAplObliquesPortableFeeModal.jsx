...
const UploadCervicalAplObliquesPortableFeeModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		if (image) {
			formData.append("cervical_apl_obliques_pf", dataURItoBlob(image));
		}
		// return "";
		Axios.post(
			`v1/imaging/cervical-apl-obliques-pf-img-result/${showData?.id}`,
			formData,
			config
		)
			.then((res) => {
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									...
									<Controller
										...
										// errors={errors?.rhu_xray_result?.message}
												...
												{hasError("cervical_apl_obliques_pf") ? (
													<p className="mt-0 text-xs text-red-600 dark:text-red-500 mb-0">
														{hasError("imacervical_apl_obliques_pfge")}
													</p>
												) : (
													""
												)}
