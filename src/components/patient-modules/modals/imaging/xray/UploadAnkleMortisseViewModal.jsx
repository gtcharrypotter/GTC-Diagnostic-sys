...
const UploadAnkleMortisseViewModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		if (image) {
			formData.append("ankle_mortisse_view_right_pf", dataURItoBlob(image));
		}
		// return "";
		Axios.post(
			`v1/imaging/ankle-mortisse-img-result/${showData?.id}`,
			formData,
			config
		)
			.then((res) => {
						...
						>
							<Dialog.Panel className="w-full lg:max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									...
									<Controller
										name="ankle_mortisse_view_right_pf"
										control={control}
										rules={{
											required: {
												value: true,
										...
										// errors={errors?.rhu_xray_result?.message}
												...
												{hasError("ankle_mortisse_view_right_pf") ? (
