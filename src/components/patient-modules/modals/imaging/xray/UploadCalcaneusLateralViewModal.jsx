...
const UploadCalcaneusLateralViewModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		if (image) {
			formData.append("calcaneus_lateral_view", dataURItoBlob(image));
		}
		// return "";
		Axios.post(
			`v1/imaging/calcaneus-lateral-img-result/${showData?.id}`,
			formData,
			config
		)
			.then((res) => {
								...
								<div className="p-6 flex flex-col gap-y-4 relative">
									...
									<Controller
										...
										// errors={errors?.rhu_xray_result?.message}
												...
												{hasError("calcaneus_lateral_view") ? (
