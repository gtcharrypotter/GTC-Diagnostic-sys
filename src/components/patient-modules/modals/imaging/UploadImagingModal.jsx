...
import { dataURItoBlob, doctorName, doctorSpecialty } from '../../../../libs/helpers';
...
const UploadImagingModal = (props, ref) => {
	...
	const submit = (data) => {
		...
		if (image) {
			formData.append("appendix_ultrasound", dataURItoBlob(image));
		}
		// return "";
		Axios.post(
			`v1/imaging/appendix-img-result/${showData?.id}`,
			formData,
			config
		)
			.then((res) => {
