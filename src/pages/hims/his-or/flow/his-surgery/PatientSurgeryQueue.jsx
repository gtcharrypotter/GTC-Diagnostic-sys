...
import AppointmentDetailsForSurgery from './components/AppointmentDetailsForSurgery';
...
import AppointmentDetailsForSurgeryOR from '../../components/AppointmentDetailsForSurgeryOR';

const PatientSurgeryQueue = () => {
  const { user } = useAuth();
				...
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-5 divide-x">
					<div className="lg:col-span-4">
						...
						<span className="noto-sans-thin text-slate-500 text-sm font-light">
							Patients pending for surgery approval
						</span>
						<div className="flex flex-col gap-y-4 py-4">
							...
							{pendingForSurgery?.data?.map((queue) => (
								<InQueueForRelease
								selected={queue?.id === appointment?.id}
...
export default PatientSurgeryQueue
