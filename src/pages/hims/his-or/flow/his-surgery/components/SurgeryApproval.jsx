...
const SurgeryApproval = (props) => {
    ...
    const sendToICU = () => {
		...
		Axios.post(`v1/hospital/send-from-surgery-to-icu/${appointment?.id}`, formdata)
			.then((response) => {
	...
	const sendToPacu = () => {
		...
		Axios.post(`v1/hospital/send-from-surgery-to-pacu/${appointment?.id}`, formdata)
			.then((response) => {
				let data = response.data;
				// console.log(data);
				setTimeout(() => {
  ...
  return (
    <div className="flex flex-col items-start">
			{appointment?.status == "pending-for-surgery-release" ? (
				<>
...
export default SurgeryApproval
