...
import { calculateAge, doctorName, formatDate, formatDateWithOffset, patientFullName, userName } from '../../../../../libs/helpers';
...
const EPresSForm = ({ patient, appointment}) => {
  ...
  const [result, setResult] = useState(null);
	...
	const handleGenerateAndSaveXML = async () => {
     setCanPrint(true);
    // try {
    //   setCanPrint(true);

    //   // Prepare data for the back-end
    //   const payload = {
    //     patient,
    //     appointment,
    ...
    //   alert("XML generated and saved successfully!");
    // } catch (error) {
    //   console.error("Error generating XML:", error);
    //   alert("Failed to generate XML.");
    // }
  };
  console.log("Prescription detatils", appointment)
  return (
    <div className="bg-gray-600 p-11 min-h-[14in]  overflow-auto phic-forms">
      {appointment?.status == "done" ? (
		...
		<ActionBtn
          ...
          size="md"
          onClick={handleGenerateAndSaveXML}
        >
        ...
        {/* <ActionBtn
          type="primary-dark"
          className={`mr-5 mb-5 py-4 !text-lg ${!downloadUrl ? "opacity-50 cursor-not-allowed" : ""}`}
          size="md"
          onClick={handleDownloadXML}
        >
          <FlatIcon icon="rr-download" className="text-white mr-1" />
          Download XML
        </ActionBtn> */}
        <ActionBtn
          type="primary-dark"
        ...
        <div className="bg-white p-[0.5in] w-[10.5in] max-w-auto gap-y-6"
          id="phic-form-printable" ref={componentRef}
        >
            <div className="bg-white border-2 !border-blue-300 flex flex-col w-[9.5in] min-h-[13in]">
            <div className="flex items-center relative justify-center gap-2  p-4">
						...
						<div className="flex flex-col text-left w-full mx-auto">
							<p className="text-sm">
								<div className='underline font-bold'>ELECTRONIC PRESCRIPTION SLIP(ePresS)</div>
							</p>
						</div>
              ...
              <div className="flex gap-4 px-4">
                    ...
                    <div className="flex text-[10px]">
                        ...
                        <span className="font-light underline">{appointment?.patient?.case_no}</span>
                    </div>
                    <div className="flex text-[10px]">
                    ...
                    <div className="flex text-[10px]">
                        ...
                        <span className="font-light underline">{appointment?.transaction_no}</span>
                    </div>
                </div>
                <div className="flex gap-4 px-4">
                    <div className="flex text-[10px] col-span-2">
                        ...
                        <span className="font-light underline">{patientFullName(patient)}</span>
                    </div>
                ...
                <div className="flex gap-4 px-4">
                        ...
                        <div className="flex text-[10px]">
                            ...
                            <span className="font-light underline">{}indigent</span>
                        </div>
                    </div>
                    <div className="flex gap-4 px-4">
                      ...
                      columns={[
                        {
                          ...
                          cell: (data) => {
                            return(
                              ...
                              <span className='text-xs'>{data?.item?.category}</span>
                            </div>
                            )}
                        },
                        {
                          header: "Medicine Strength/Form/Volume (Gamot/Anyo/Dami)",
                          className: "text-left",
                          tdClassName: "text-left",
                          key: "medicine",
                          cell: (data) =>  {
                            return(
                              <div className="flex flex-col justify-center items-center">
                                <span className='text-xs'>{data?.item?.name} {data?.item?.unit_measurement}</span>
                              </div>
                            )
                          }
                        },
                        {
                          header: "Quality (Bilang)",
                          className: "text-center",
                        ...
                        {
                          header: "Name of the Prescribing Physician (Pangalan ng nagresetang doktor)",
                          className: "text-left",
                          tdClassName: "text-left",
                          key: "doctor",
                          cell: (data) => {
                            return(
                              <div className="flex flex-col justify-center items-center">
                                  <span className='text-xs'>
                                    {doctorName(appointment?.prescribedByDoctor)}
                                  </span>
                                  <span className='text-[8px]'>
                                  Signature over printed name
                                  </span>
                                  <span className='text-[10px]'>
                                    License #: {appointment?.prescribedByDoctor?.accreditation_number}
                                  </span>
                              </div>
                            );
                          },
                        ...
                        {
                          header: "Dispensed (nabigay) Not dispensed (hindi naibigay)",
                          className: "text-center",
                          tdClassName: "text-center",
                          key: "dispensed",
                          cell: (data) => <Status status={data?.status} />,
                        },
                        {
                          header: "Date dispensed (Petsa kung kelan naibigay)",
                          className: "text-center",
                          tdClassName: "text-center",
                          key: "date_dispensed",
                          cell: (data) => formatDate(data?.updated_at),
                        },
                        {
                          header: "Name of the Dispensing Personnel (Pangalan ng nagbigay)",
                          className: "text-left",
                          tdClassName: "text-left",
                          key: "dispensing_personnel",
                          cell: (data) => {
                            return(
                              <div className="flex flex-col justify-center items-center">
                                  <span className='text-[10px]'>
                                    {userName(appointment?.releasedBy)}
                                  </span>
                                  <span className='text-[8px]'>
                                  Signature over printed name
                                  </span>
                                  <span className='text-[8px]'>
                                    Facility: {appointment?.releasedBy?.healthUnit?.name}
                                  </span>
                              </div>
                            );
                          },
