...
import { accessWasteOptions, diabetesSymptoms, diarrheaSymptoms, environmentalHistories, generalConsultation, generalHistories, hypertensionSymptoms, infectiousSymptoms, lRISymptoms, malariaSymptoms, medicalSurgicalHistories, sanitaryOptions, symptoms, symptoms2, uRISymptoms } from '../../../../libs/appointmentOptions';
...
const CreateEmergencyCareModal = (props, ref) => {
    const { onSuccess, patientSelfie, referToRHURef, appointment, referToERInfectiousRef } = props;
	...
	const referPatientToInfectious = (data) => {
		setLoading(true);
		...
		Axios.post(`v1/hospital/refer-to-doctor/${modalData?.id}`, formdata)
			.then((response) => {
										...
										{modalData?.patient ? (
											<div className="lg:col-span-12 flex flex-col pb-4">
												...
												<div className="flex gap-4 ml-auto">
													...
													<span className="text-gray-900 text-center">{modalData?.patient?.philhealth}</span>
									...
									{hasSymptoms > 2 ? (
										<div className="lg:col-span-12">
											<h4 className="border-y-2 text-base font-bold p-2 mb-4 lg:col-span-12">
												<span className='text-red-700 animate-none'>REFER TO INFECTIOUS ER NURSE</span>
								...
								<div className="px-4 py-4 flex items-center justify-end bg-slate- border-t">
									{hasSymptoms ? (
										<ActionBtn
											...
											onClick={() => {
																if (
																	referToERInfectiousRef
																) {
																	referToERInfectiousRef?.current.show(
																		{
																			data: modalData
																			// fn: referPatientToInfectious,
																		}
																	);
																	hide();
																}
