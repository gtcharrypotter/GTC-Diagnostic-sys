...
import InQueueForDischarge from "./components/InQueueForDischarge";

const PatientCashierQueue = () => {
	const { user } = useAuth();
	const { pending, mutatePending, discharge, mutateDischarge } = useCashierQueue();
				...
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-5 divide-x">
					<div className="lg:col-span-3">
						...
						<div className="flex flex-col gap-y-4 py-4">
							...
							{discharge?.data?.map((queue, index) => {
								return (
									<InQueueForDischarge
										selected={queue?.id == appointment?.id}
										onClick={() => {
